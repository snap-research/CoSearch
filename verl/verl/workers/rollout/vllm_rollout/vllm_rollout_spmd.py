...
class vLLMAsyncRollout(BaseRollout):
    ...
    def _init_zeromq(self) -> str:
        ...
        # Delay loop task creation until we have an event loop (in async context)
        self.zmq_loop_task = None
        try:
            loop = asyncio.get_running_loop()
            self.zmq_loop_task = loop.create_task(self._loop_forever())
        except RuntimeError:
            # No running event loop yet, will be started later in ensure_zmq_loop_started()
            pass

        return address
    
    def ensure_zmq_loop_started(self):
        """Ensure ZeroMQ loop task is started. Called when we have an event loop."""
        if self.zmq_loop_task is None:
            try:
                loop = asyncio.get_running_loop()
                self.zmq_loop_task = loop.create_task(self._loop_forever())
            except RuntimeError:
