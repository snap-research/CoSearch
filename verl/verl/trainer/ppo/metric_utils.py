...
def compute_data_metrics(batch: DataProto, use_critic: bool = True, agent_name: str = "") -> dict[str, Any]:
    """
    ...
    Args:
        ...
        agent_name: Optional prefix for metric keys (e.g., "main", "reranker"). 
                   If empty (default), metrics have no prefix. If provided, all keys will be 
                   prefixed with "{agent_name}_". Defaults to "".
    ...
    # Add agent_name prefix to all keys if agent_name is provided
    if agent_name:
        metrics = {f"{agent_name}_{key}": value for key, value in metrics.items()}
