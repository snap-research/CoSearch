...
class SearchR1ToolParser(ToolParser):
    """Tool parser for Search R1 format.
    
    Extracts tool calls from the format specified in SEARCH_R1_PROMPT:
    ...
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:
        ...
        for match in matches:
            try:
                ...
                function_call = json.loads(match_stripped)
                name = function_call.get("name")
                arguments = function_call.get("arguments", {})
                
                # Validate it's a search call (as per SEARCH_R1_PROMPT spec)
                if name == "search":
                    # Ensure arguments is a dict and serialize it
                    if not isinstance(arguments, dict):
                        logger.warning(f"Expected arguments to be dict, got {type(arguments)}")
                        arguments = {}
                    ...
                    logger.warning(f"Expected tool name 'search', got '{name}'. Skipping this tool call.")
            except json.JSONDecodeError as e:
                logger.error(f"Failed to decode tool call JSON: {e}. Content: {match[:200]}")
            except Exception as e:
                logger.error(f"Failed to parse tool call: {e}")

        # Remove tool call tags from content (keep <reason> and <answer>)
        content = self.tool_call_regex.sub("", text)
