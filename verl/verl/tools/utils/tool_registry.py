...
def initialize_tools_from_config(tools_config_file):
    print("\n" + "="*80)
    print("[TOOL CONFIG VALIDATION] Loading tool configuration")
    print(f"Tool config file: {tools_config_file}")
    tools_config = OmegaConf.load(tools_config_file)
    print(f"Tool config content:")
    print(OmegaConf.to_yaml(tools_config))
    print("="*80 + "\n")
    tool_list = []
        ...
        for tool_config in tools_config.tools:
            ...
            match tool_type:
                case ToolType.NATIVE:
                    ...
                    print(f"\n[TOOL CONFIG VALIDATION] Initializing tool: {cls_name}")
                    print(f"Tool type: {tool_type}")
                    print(f"Tool config: {tool_config_dict}")
                    tool = tool_cls(
                        config=tool_config_dict,
                        tool_schema=tool_schema,
                    )
                    print(f"Tool name: {tool.name}")
                    print(f"Tool initialized successfully: {tool.__class__.__name__}\n")
                    tool_list.append(tool)
                case ToolType.MCP:
